...
impl Index<HashMap<String,Vec<usize>>,Index8ExtraVariables> {
    ...
    pub fn boolean_search_demorgan(&self, exp: &String) -> Option<Vec<String>>{
        match Expr::from_string(&exp) {
            Ok(Expr(ExprData::HasNodes(node))) => self.vec_to_articleset(self.evaluate_syntex_tree_demorgan(node)),
    ...
    pub fn evaluate_syntex_tree_demorgan(&self, node: AstNode)-> Vec<usize> {
        match node {
            ...
            match (*left_child, *right_child){
                (AstNode::Invert(left_child),AstNode::Invert(right_child))=> self.invert(self.and(self.evaluate_syntex_tree_demorgan(*left_child),self.evaluate_syntex_tree_demorgan(*right_child))),
...
mod tests {
    ...
    fn search_match (
        ...
        dbg!(&query.to_string());
        let index_result: HashSet<String> = HashSet::from_iter(index.boolean_search_demorgan(&query.to_string()).unwrap_or(Vec::default()));
