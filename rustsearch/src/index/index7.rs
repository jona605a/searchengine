...
impl Index<HashMap<String, Vec<usize>>, Index7ExtraVariables> {
    pub fn index7(
        config: &Config,
    ) -> Result<Index<HashMap<String, Vec<usize>>, Index7ExtraVariables>, Box<dyn Error>> {
    ...
    pub fn evaluate_syntax_tree(&self, node: AstNode) -> Vec<usize> {
        match node {
...
mod tests {
    ...
    fn setup_real() -> Index<HashMap<String, Vec<usize>>, Index7ExtraVariables> {
    ...
    fn setup_test() -> Index<HashMap<String, Vec<usize>>, Index7ExtraVariables> {
        let mut database: HashMap<String, Vec<usize>> = HashMap::new();
    ...
    fn search_match(
        index: &Index<HashMap<String, Vec<usize>>, Index7ExtraVariables>,
        query: &str,
        titles: Vec<&str>,
    ) {
        dbg!(&query.to_string());
        let index_result: HashSet<String> = HashSet::from_iter(
            index
                .boolean_search(&query.to_string())
